...
resource "aws_security_group" "ec2-database-sg443" {
...
#latest image  
data "aws_ami" "latest-amazon-linux-image-database" {
    most_recent = true
    owners = ["amazon"]
    filter {
        name = "name"
...
resource "aws_instance" "ec2-database" {
  ami = data.aws_ami.latest-amazon-linux-image-database.id
 ...
 user_data = base64encode(<<-EOF
  ...
  vpc_security_group_ids = [aws_security_group.ec2-database-sg443.id]
...
output "database_public_ip" {
  value = aws_instance.ec2-database.public_ip
}
